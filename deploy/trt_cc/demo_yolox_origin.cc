...
const char *INPUT_BLOB_NAME = "batched_inputs";
...
cv::Mat static_resize(cv::Mat &img) {
  ...
  //   cv::Mat re(unpad_w, unpad_h, CV_8UC3);
...
static void
generate_grids_and_stride(const int target_size_w, const int target_size_h,
                          std::vector<int> &strides,
                          std::vector<GridAndStride> &grid_strides) {
  for (auto stride : strides) {
    int num_grid_0 = target_size_w / stride;
    int num_grid_1 = target_size_h / stride;
    std::cout << num_grid_0 << " " << num_grid_1 << " " << stride << std::endl;
    for (int g1 = 0; g1 < num_grid_1; g1++) {
      for (int g0 = 0; g0 < num_grid_0; g0++) {
        grid_strides.push_back((GridAndStride){g0, g1, stride});
      }
    }
  }
...
static void generate_yolox_proposals(std::vector<GridAndStride> grid_strides,
  ...
  std::cout << "num_a: " << num_anchors << std::endl;
...
static void decode_outputs(float *prob, std::vector<Object> &objects,
                           float scale, const int img_w, const int img_h) {
  std::vector<Object> proposals;
  std::vector<int> strides = {8, 16, 32};
  std::vector<GridAndStride> grid_strides;
  //   generate_grids_and_stride(INPUT_H, INPUT_W, strides, grid_strides);
...
int main(int argc, char **argv) {
  ...
  if (argc == 4 && std::string(argv[2]) == "-i") {
    ...
    std::cerr << "run 'python3 yolox/deploy/trt.py -n yolox-{tiny, s, m, l, "
                 "x}' to serialize model first!"
              << std::endl;
    std::cerr << "Then use the following command:" << std::endl;
    std::cerr << "./yolox ../model_trt.engine -i ../../../assets/dog.jpg  // "
                 "deserialize file and run inference"
              << std::endl;
    return -1;
  }
  ...
  cv::Mat inp = cv::Mat::zeros(pr_img.size(), CV_32FC3);
  pr_img.convertTo(inp, CV_32FC3);

  float *blob = new float[3 * inp.cols * inp.rows];
  memcpy(blob, inp.data, 3 * inp.cols * inp.rows * sizeof(float));
